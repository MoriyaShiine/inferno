...
public class FireBlockMixin {
	...
	private boolean inferno$charredLogs(Level instance, BlockPos pos, boolean movedByPiston, Operation<Boolean> original, @Local(name = "oldState") BlockState oldState) {
		if (InfernoConfig.charredLogs && instance.getRandom().nextBoolean() && oldState.is(BlockTags.LOGS)) {
			BlockState charredState = InfernoBlocks.CHARRED_LOG.defaultBlockState();
